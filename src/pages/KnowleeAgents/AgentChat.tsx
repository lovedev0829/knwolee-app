...
export default function AgentChat() {
  ...
  const firstQuestion = (location.state as { question: string })?.question || "";
  ...
  useEffect(() => {
    const locationState = location.state as { assistantId?: string };
    const assistantId = locationState?.assistantId;
    if (assistantId) {
      setAgentId(assistantId);
      // find user agent with assistant id
      const userAgent = userAgents?.find(
        (agent) => agent?.assistant?.id === assistantId
      );
      if (userAgent) {
        // set agent name
        setAgentName(userAgent?.assistant?.name || "");
      }
    }
  }, [location.state, userAgents]);
      ...
      >
        ...
        >
          <Flex alignItems="center" justifyContent="space-between">
            ...
            {/* hide mobile menu drawer for AI Assistants - Chat */}
