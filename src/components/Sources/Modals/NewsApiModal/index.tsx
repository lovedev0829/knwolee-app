...
import { newsSubSetTypes } from "../../../../utils/constants";
import { EntityPayload, SOURCE_TYPES } from "../../../../utils/types";
import { validateNewsKeyword, validateNewsUrl } from "../../../../utils/urlValidation";
import {
  AddButton,
  BulkUploadButton,
  CancelButton,
...
export type NewsEntityPayload = EntityPayload & {
  id: string;
  subSetType: string;
  placeholder: string;
...
const emptyNewsEntity = {
  id: uuidv4(),
  value: "",
  sourceType: SOURCE_TYPES.NEWSAPI,
  subSetType: "",
  isScraped: false,
  placeholder: "",
  placeholderURL: "",
...
const NewsModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [entities, setEntities] = useState<NewsEntityPayload[]>([
    emptyNewsEntity,
  ]);
  ...
  const validateEntities = () => {
    ...
    for (const entity of entities) {
      ...
      if(entity.subSetType === "keyword") {
        const isValidKeyword = validateNewsKeyword(entity.value)
        if(!isValidKeyword) {
          setError(entity.validationMessage || "Please insert a value");
          return true;
        }
      } else {
        const isValidURL = entity.value.includes("https:") || entity.value.includes("http:")
        if(!isValidURL) {
          setError(entity.validationMessage || "Please insert a value");
          return true;
        }
  ...
  const addNewEmptyEntity = () => {
    ...
    const newEntity = { ...emptyNewsEntity, id: uuidv4() };
  ...
  return (
    <Box>
      <ModalBody p={0}>
        ...
        />
        <Description text={MODALS_DESCRIPTIONS.NEWSAPI} />
