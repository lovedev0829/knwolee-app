...
import { Box, ModalBody, ModalFooter, Text, useColorMode, useColorModeValue, useToast } from "@chakra-ui/react";
...
import { mediumSubSetTypes } from "../../../../utils/constants";
import { EntityPayload, SOURCE_TYPES } from "../../../../utils/types";
import { validateMediumArticle, validateAlphanumericWithNoSlash } from "../../../../utils/urlValidation";
import {
  AddButton,
  BulkUploadButton,
  CancelButton,
...
export type MediumEntityPayload = EntityPayload & {
  id: string;
  subSetType: string;
  placeholder: string;
...
const emptyMediumEntity = {
  id: uuidv4(),
  value: "",
  sourceType: SOURCE_TYPES.MEDIUM,
  subSetType: "",
  isScraped: false,
  placeholder: "",
  placeholderURL: "",
...
const MediumModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [entities, setEntities] = useState<MediumEntityPayload[]>([
    emptyMediumEntity,
  ]);
  ...
  const validateEntities = () => {
    ...
    for (const entity of entities) {
      ...
      if (entity.subSetType === "article") {
        const isValidMediumProfileAndArticle = validateMediumArticle(entity.value);
        if (!isValidMediumProfileAndArticle) {
          setError(entity.validationMessage || "Please insert a valid URL");
          return true;
        }
      } else {
        const isValidMediumUsername = validateAlphanumericWithNoSlash(entity.value);
        if (!isValidMediumUsername) {
  ...
  const addNewEmptyEntity = () => {
    ...
    const newEntity = { ...emptyMediumEntity, id: uuidv4() };
  ...
  return (
    <Box>
      <ModalBody p={0}>
        ...
        />
        <Description text={MODALS_DESCRIPTIONS.MEDIUM} />
