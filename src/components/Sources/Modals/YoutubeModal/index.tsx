...
import { youtubeSubSetTypes } from "../../../../utils/constants";
import { EntityPayload, SOURCE_TYPES } from "../../../../utils/types";
import { validateYoutubeAccountUsername, validateYoutubeVideoId } from "../../../../utils/urlValidation";
import {
  AddButton,
  BulkUploadButton,
  CancelButton,
...
export type YoutubeEntityPayload = EntityPayload & {
  id: string;
  subSetType: string;
  placeholder: string,
...
const YoutubeModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [entities, setEntities] = useState<YoutubeEntityPayload[]>([
    emptyYoutubeEntity,
  ]);
  ...
  const validateEntities = () => {
    ...
    for (const entity of entities) {
      ...
      if (entity.subSetType === "video") {
        const isValidVideoId = validateYoutubeVideoId(entity.value);
        if (!isValidVideoId) {
          setError(entity.validationMessage);
          return true;
        }
      }

      if (entity.subSetType === "account") {
        const isValidUsername = validateYoutubeAccountUsername(entity.value);
        if (!isValidUsername) {
          setError(entity.validationMessage);
          return true;
        }
  ...
  const addNewEmptyEntity = () => {
    ...
    const newEntity = { ...emptyYoutubeEntity, id: uuidv4() };
  ...
  return (
    <Box>
      <ModalBody p={0}>
        ...
        />
        <Description text={MODALS_DESCRIPTIONS.YOUTUBE} />
        ...
        <Box px={1} mx={-1} flexGrow={1}>
          {entities.map((entity, index) => (
            <StyledInput
              ...
              subSetTypes={youtubeSubSetTypes}
