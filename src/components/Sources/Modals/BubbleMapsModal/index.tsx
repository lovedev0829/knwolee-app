...
import { bubblemapsSubSetTypes } from "../../../../utils/constants";
...
import BubblemapsIcon from "src/Icons/BubblemapsIcon";

type Props = {
  onClose: () => void
}

function BubblemapsModal({ onClose }: Props) {
  ...
  const emptyEntity = {
    id: uuidv4(),
    value: "",
    sourceType: SOURCE_TYPES.BUBBLEMAPS,
    subSetType: "",
    isScraped: false,
    placeholder: "",
    placeholderURL: "",
  ...
  const isDisabledAddMore = entities.length >= 5;
    ...
    <Box>
      <ModalBody p={0}>
        <TitleWithIcon
          ...
          iconColor="#F12489"
        />
        <Description text={MODALS_DESCRIPTIONS.BUBBLEMAPS} />
        ...
        <Box px={1} mx={-1} flexGrow={1}>
          {entities.map((entity, index) => (
            <StyledInput
              ...
              subSetTypes={bubblemapsSubSetTypes}
...
export default BubblemapsModal;
