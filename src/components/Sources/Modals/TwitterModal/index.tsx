...
import { twitterSubSetTypes } from "../../../../utils/constants";
import { EntityPayload, SOURCE_TYPES } from "../../../../utils/types";
import { validateCommonURLCharsWithSlash, validateAlphanumericWithNoSlash, validateAlphanumericWithSlash, validateCommonURLCharsWithoutSlash } from "../../../../utils/urlValidation";
import {
  AddButton,
  BulkUploadButton,
  CancelButton,
...
export type TwitterEntityPayload = EntityPayload & {
  id: string;
  subSetType: string;
  placeholder: string,
...
const TwitterModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const emptyTwitterEntity = {
    id: uuidv4(),
    value: "",
    sourceType: SOURCE_TYPES.TWITTER,
    subSetType: "",
    isScraped: false,
    placeholder: "",
    placeholderURL: "",
  ...
  const [entities, setEntities] = useState<TwitterEntityPayload[]>([
    emptyTwitterEntity,
  ]);
  ...
  const validateEntities = () => {
    ...
    for (const entity of entities) {
      ...
      if(entity.subSetType === "tweet" || entity.subSetType === "thread") {
        const isTweetOrThread = validateCommonURLCharsWithSlash(entity.value);
        if (!isTweetOrThread || !entity.value.includes("status")) {
          setError(entity.validationMessage || "Please insert a valid URL");
          return true;
        }
      } else if(entity.subSetType === "profile") {
        const isProfile = validateCommonURLCharsWithoutSlash(entity.value);
        if (!isProfile) {
          setError(entity.validationMessage || "Please insert a valid username");
          return true;  
        }
      } else if(entity.subSetType === "hashtag") {   
        const isHashTag = validateAlphanumericWithNoSlash(entity.value);
        if (!isHashTag) {
          setError(entity.validationMessage || "Please insert a valid input");
          return true;
        }
      }
  ...
  const addNewEmptyEntity = () => {
    ...
    const newEntity = { ...emptyTwitterEntity, id: uuidv4() };
  ...
  return (
    <Box>
      <ModalBody p={0}>
        ...
        />
        <Description text={MODALS_DESCRIPTIONS.TWITTER} />
        ...
        <Box px={1} mx={-1} flexGrow={1}>
          {entities.map((entity, index) => (
            <StyledInput
              ...
              subSetTypes={twitterSubSetTypes}
